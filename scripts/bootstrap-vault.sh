...
initVault() {
  ...
  if [ "$init_status" == "false" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        echo "darwin system"
        sed -i '' "s~VAULT_ROOT_TOKEN=\".*\"~VAULT_ROOT_TOKEN=\"$VAULT_ROOT_TOKEN\"~" "$REPO_ROOT"/scripts/.env
        sed -i '' "s~VAULT_RECOVERY_TOKEN=\".*\"~VAULT_RECOVERY_TOKEN=\"$VAULT_RECOVERY_TOKEN\"~" "$REPO_ROOT"/scripts/.env
    else
...
loginVault() {
  ...
  if [ -z "$VAULT_ROOT_TOKEN" ]; then
    echo "VAULT_ROOT_TOKEN is not set! Check $REPO_ROOT/scripts/.env"
    exit 1
  fi
...
EOF
  ...
  #vault delete auth/kubernetes/config
...
loadSecretsToVault() {
  ...
  vault kv put secrets/ambassador/ambassador-agent-key cloudConnectToken="$AMBASSADOR_CLOUD_CONNECT_TOKEN"

  ####################
  # helm chart values
  ####################
  kvault "infrastructure/kured/kured-helm-values.txt"
  kvault "infrastructure/logs/loki/loki-helm-values.txt"
  kvault "infrastructure/monitoring/botkube/botkube-helm-values.txt"
  kvault "infrastructure/monitoring/grafana/grafana-helm-values.txt"
  kvault "infrastructure/monitoring/kube-prometheus-stack/kube-prometheus-stack-helm-values.txt"
  kvault "infrastructure/monitoring/thanos/thanos-helm-values.txt"
  kvault "infrastructure/monitoring/uptimerobot-prometheus/uptimerobot-prometheus-helm-values.txt"
  kvault "apps/emqx/emqx-helm-values.txt"
  kvault "apps/home-assistant/home-assistant-helm-values.txt"
  kvault "apps/plex/plex-helm-values.txt"
  kvault "apps/rtorrent-flood/rtorrent-flood-helm-values.txt"
  kvault "infrastructure/velero/velero-helm-values.txt"
  kvault "infrastructure/ingress/ambassador-helm-values.txt"
}
